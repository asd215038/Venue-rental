<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    <!-- 篩選器 -->
    <div
      class="bg-white rounded-lg shadow-md p-4 mb-8 flex flex-wrap items-center gap-4"
    >
      <select
    ...
    <!-- 載入狀態 -->
    <div v-if="isLoading" class="text-center py-8">載入中...</div>
    ...
    >
      ...
      >
        <img
          :src="'/src/assets/images/venue/' + venue.imageUrl"
          alt="場地圖片"
        ...
        <div class="p-4 flex-1 flex flex-col">
          ...
          <p class="text-sm text-gray-600 mb-4">
            {{ venue.description || "暫無描述" }}
          </p>
          <p class="text-sm text-gray-800 font-medium">
            NT$ {{ venue.price_per_hour }}/小時
          </p>
          <p class="text-sm text-gray-800 font-medium">
            容納人數: {{ venue.capacity }}
          </p>
    ...
    <!-- 場地詳細資訊模態框 -->
    <div
      v-if="isModalOpen && selectedVenue"
      class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
    >
      <div class="bg-white rounded-lg p-6 max-w-lg w-full">
        <h3 class="text-lg font-bold text-gray-800 mb-2">
          {{ selectedVenue.venues_name }}
        </h3>
        <img
          :src="'/src/assets/images/venue/' + selectedVenue.imageUrl"
          alt="場地圖片"
          class="h-48 w-full object-cover mb-4"
        />
        <p class="text-sm text-gray-600 mb-4">
          {{ selectedVenue.description || "暫無描述" }}
        </p>
        <p class="text-sm text-gray-800 font-medium">
          NT$ {{ selectedVenue.price_per_hour }}/小時
        </p>
        <p class="text-sm text-gray-800 font-medium">
          容納人數: {{ selectedVenue.capacity }}
        </p>
...
export default {
  ...
  computed: {
    ...
    displayVenues() {
      ...
      return this.filteredVenues.length > 0 ? this.filteredVenues : this.venues;
    },
  },
  ...
  methods: {
    ...
    filterVenues() {
      ...
      this.filteredVenues = this.venues.filter((venue) => {
        // Category filter
        const categoryMatch =
          !this.selectedCategory || venue.category === this.selectedCategory;
        ...
        // Combine all filters
        return categoryMatch && capacityMatch && keywordMatch;
      });
