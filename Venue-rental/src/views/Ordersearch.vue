    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          ...
          <tr>
            <th v-for="(header, index) in tableHeaders" :key="index" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ header }}
            </th>
          </tr>
          </thead>
      ...
      <div class="bg-white rounded-lg shadow-lg w-full max-w-md mx-4">
        <div class="p-6">
          ...
          <div class="mb-6">
            ...
            <p><strong>訂單狀態:</strong> {{ orderDetails.status }}</p>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      orderDetails: {}, // 用來顯示訂單詳情
      orderIdCounter: 1, // 設置自動生成的訂單ID起始值
    };
  },
  async created() {
  ...
  methods: {
    ...
    async fetchUserOrders() {
      ...
      try {
        ...
        const ordersQuery = query(
            ordersRef,
            where("reserve_user", "==", currentUser.email) // 根據使用者的電子郵件查詢訂單
        );
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          ordersList.push({
            order_id: this.orderIdCounter++, // 自動生成訂單ID
            reserve_venue: data.reserve_venue,
            reserve_date: data.reserve_date,
            reserve_time: data.reserve_time,
            total_amount: data.reserve_price, // 假設 reserve_price 是每小時價格，可能需要進一步處理計算總金額
            status: data.status || "待支付", // 假設訂單狀態欄位存在
          });
        });
    ...
    async requestCancellation(orderId) {
      ...
      try {
        const orderRef = doc(db, "reservations", order.order_id.toString());
        await updateDoc(orderRef, {status: "待處理取消"}); // 設定為待處理取消
        order.status = "待處理取消"; // 更新本地狀態
        alert("取消請求已送出，請等待管理者處理");
      } catch (error) {
        console.error("發送取消請求時發生錯誤:", error);
        alert("無法送出取消請求，請稍後再試");
      }
    },
  },
};
